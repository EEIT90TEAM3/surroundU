...
<!-- 開啟新視窗查看文章內容 -->
 <script type="text/javascript">
		 function openWindow()
		 {
		 mynewwindow = window.open('<c:url value="/lookAccuse.controller?accuse_no=${param.accuse_no}&accuse_topic=${param.accuse_topic}"/>','_new','scrollbars=yes,status=yes,width=800,height=600,dependent=no,directories=no,menubar=yes,personalbar=no'); 
		 } 
...
<%-- 			<form mothed="get" action="<c:url value="/lookAccuse.controller"/>"  >	 --%>
				   ...
				   <table class="table table-bordered table-striped table-hover">		
					...
					<!-- <th><input type="button"  value="觀看文章內容" id="buttonLook"></th> -->	
						...
						<tr>
						   <th>處理結果</th>
						   <c:if test="${param.accuse_status==2}">
						   <th>檢舉成立</th>
				  		   </c:if>
				  		   <c:if test="${param.accuse_status==1}">
						   <th>檢舉不成立</th>
				  		   </c:if>
				        </tr>
						<tr> 
